...
module Intcode
  ...
  -- * Special 'IMInterpreter' for hull painting robot's brain.
  , executeUntilOutputOrHalted
  , mkIMEnv
  , runIMInterpreterUntilOutputOrHalted
...
executeUntilOutputOrHalted ::
  ...
  addr -> m (Maybe addr)
executeUntilOutputOrHalted ip0 =
  go =<< (traceMsg ("ip0: " ++ show ip0) *> decodeOpAt ip0)
  where
    go (op, mIp) = do
      traceMsg ("op: " ++ show op ++ ", nextIp: " ++ show mIp)
      c <-
        interpretOp op
      case (c, mIp, opIsOutput op) of

        (Continue, Just _, True) ->
          pure mIp

        (Continue, Just ip, False) ->
          decodeOpAt ip >>= go
...
mkIMEnv :: [val] -> Int -> IMEnv val
mkIMEnv prog relativeBase =
  env
  where
    env =
      IMEnv mem relativeBase mempty mempty mempty
    mem =
      IntMap.fromList (zip [0..] prog)
...
runIMInterpreterUntilOutputOrHalted ::
  ...
  IMEnv val -> Int -> Either String (Maybe (Int, val), IMEnv val)
runIMInterpreterUntilOutputOrHalted env startAddr =
  fixup =<< run
  where
    -- Remove output from state and yield it separately. This is to
    -- make it harder to erroneously restart the program from a state
    -- which contains output. Also verify that only a single output
    -- was produced.
    fixup (mAddr, newEnv) =
      case mAddr of
        Nothing ->
          -- halted
          Right (Nothing, newEnv)
        Just addr ->
          -- produced output
          case imOutput newEnv of
            [val] ->
              Right (Just (addr, val), newEnv { imOutput = mempty } )
            [] ->
              Left $
    ...
    run =
      Except.runExcept
      $ flip State.runStateT env
      $ runIMInterpreterInner
      $ (executeUntilOutputOrHalted startAddr :: IMInterpreter 'NotTracing val (Maybe Int))
